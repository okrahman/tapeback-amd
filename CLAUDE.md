...
- The LLM summarizer is the default outbound text seam, and a meeting transcript is personal data. With the fully-local `faster-whisper` backend (`TAPEBACK_TRANSCRIPTION_BACKEND=faster-whisper`), `summarizer.summarize()` is the single outbound call and the single text PII masking seam (`_mask.py`) — a new outbound call must go through it, never past it. Masking is opt-in (`TAPEBACK_MASK_PII`) because it is the user's data and their call. With the default `lemonade` transcription backend (`TAPEBACK_TRANSCRIPTION_BACKEND` defaults to `lemonade`), raw meeting audio (WAV chunks) leaves the machine on every run to the user-configured Lemonade server over HTTPS (or HTTP for strictly recognized loopback endpoints with proxy bypass); text PII masking is not applicable to raw audio before text exists. This default-on activation is deliberate, and the accompanying opt-out (`TAPEBACK_TRANSCRIPTION_BACKEND=faster-whisper` keeps transcription fully local) must stay documented wherever the default is. The Lemonade trust boundary requires: prominent disclosure of the default-on activation (README, CHANGELOG, and runtime status — every run names the active backend and, for Lemonade, the endpoint it sends audio to), documented opt-out, no redirect following to avoid auth/audio exfiltration, and strict credential/secret redaction from logs and error messages.
