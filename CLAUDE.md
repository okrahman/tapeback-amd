...
- The LLM summarizer is the default outbound seam, and a meeting transcript is personal data. In the default local setup (`faster-whisper`), `summarizer.summarize()` is the single outbound call and the single text PII masking seam (`_mask.py`) — a new outbound call must go through it, never past it. Masking is opt-in (`TAPEBACK_MASK_PII`) because it is the user's data and their call. When the user explicitly configures the opt-in `lemonade` transcription backend (`TAPEBACK_TRANSCRIPTION_BACKEND=lemonade`), raw meeting audio (WAV chunks) leaves the machine to the user-configured Lemonade server over HTTPS (or HTTP for strictly recognized loopback endpoints with proxy bypass); text PII masking is not applicable to raw audio before text exists. The Lemonade trust boundary requires: opt-in activation, prominent documentation/disclosure, no redirect following to avoid auth/audio exfiltration, and strict credential/secret redaction from logs and error messages.
