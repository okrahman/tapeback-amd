...
- The LLM summarizer is the default outbound text seam, and a meeting transcript is personal data. The default transcription backend is `faster-whisper` (`TAPEBACK_TRANSCRIPTION_BACKEND` defaults to `faster-whisper`), which is fully local: `summarizer.summarize()` is then the single outbound call and the single text PII masking seam (`_mask.py`) — a new outbound call must go through it, never past it. Masking is opt-in (`TAPEBACK_MASK_PII`) because it is the user's data and their call. The `lemonade` backend is explicit opt-in (`TAPEBACK_TRANSCRIPTION_BACKEND=lemonade`): raw meeting audio (WAV chunks) then leaves the machine on every run while it is selected, heading to the user-configured Lemonade server over HTTPS (or HTTP for strictly recognized loopback endpoints with proxy bypass); text PII masking is not applicable to raw audio before text exists. The Lemonade trust boundary requires: prominent disclosure of the active backend and — for Lemonade — the endpoint audio is sent to, on every run (README, CHANGELOG, and runtime status), a documented opt-out (`faster-whisper` keeps transcription fully local), no redirect following to avoid auth/audio exfiltration, and strict credential/secret redaction from logs and error messages. **Lemonade must never become the default backend**: default-on audio upload is a trust-boundary regression, not a feature.
