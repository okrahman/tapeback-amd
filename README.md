...
| `TAPEBACK_HOTWORDS` | *(project glossary)* | Comma-separated terms decoding is biased towards. Ships with a software-meeting glossary (RAG, ONNX, LLM, Obsidian, Jira, …). Whisper mangles English terms embedded in Russian speech — "tapeback" came back as "ты пупа ты бэк"; with the glossary it comes back as "tapeback". Replace it with your own domain vocabulary; empty disables the bias |
