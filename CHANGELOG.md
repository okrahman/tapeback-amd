...
### Added
- **Lemonade transcription backend (opt-in)**: transcription can run through a [Lemonade Server](https://github.com/lemonade-sdk/lemonade) you run yourself — opt in with `TAPEBACK_TRANSCRIPTION_BACKEND=lemonade` (point tapeback at the URL, optionally set a model and bearer token, and keep everything else — server lifecycle, hardware, accelerator — on the server side). The default backend remains `faster-whisper`, which is fully local; **raw meeting audio leaves the machine only after this explicit opt-in, never by default**. Tapeback sends multipart `POST /v1/audio/transcriptions` requests (`response_format=verbose_json`), chunks long WAVs with conservative internal duration/byte bounds and a small contextual overlap, deduplicates chunks against a versioned core-interval policy, detects the language from the first chunk that contains speech (normalizing Whisper names to ISO-639-1) and pins it for later chunks, and never encodes the server's accelerator choice in configuration, logs, or cache keys. The API key (`TAPEBACK_LEMONADE_API_KEY`) goes only into the `Authorization` header — never into logs, error messages, or resume-cache fingerprints. (Superseded below: `lemonade` is now the default backend.)
...
### Changed
- **`lemonade` is the default transcription backend again — this time deliberately; faster-whisper is the explicit local opt-out.** Supersedes the change below that restored `faster-whisper` as the default: shipping transcription through the user-configured Lemonade Server by default is now the intended behavior, including on a local multi-user host (owner decision, 2026-09-06). Raw recording audio therefore leaves the machine on every default run — the per-run disclosure line (including the "unauthenticated plaintext" warning) still prints on every run, and the local port-squatting caveat it describes still applies. Set `TAPEBACK_TRANSCRIPTION_BACKEND=faster-whisper` to keep transcription fully local; eligible Lemonade failures still fall back to faster-whisper automatically, and the regression pins in `tests/regressions/test_default_backend.py` now guard the new default in both directions.
- **`faster-whisper` is the default transcription backend again; Lemonade is explicit opt-in.** An earlier iteration of this release shipped Lemonade as the default, which sent raw recording audio to plaintext `http://127.0.0.1:13305` with an optional bearer token — on a multi-user host where the real server is absent, any unprivileged local process could bind the predictable port first and silently receive microphone/system audio, with plausible JSON making the capture look successful. The default is back to fully-local faster-whisper; set `TAPEBACK_TRANSCRIPTION_BACKEND=lemonade` to opt in, and the CLI's "local transcription" description matches reality again.
...
- faster-whisper logic moved to `tapeback._fw_backend` and Lemonade to `tapeback._lemonade` behind a minimal `TranscriptionBackend` protocol (`tapeback._backends`); choosing Lemonade no longer pays the ~10s faster-whisper import. All previous `Transcriber` settings and environment variables are unchanged — **the default backend is now `lemonade`** (see above) — set `TAPEBACK_TRANSCRIPTION_BACKEND=faster-whisper` to opt out
...
- **Lemonade BPE tokens are no longer exposed as words.** Lemonade segment text and
  ...
  filtering and speaker attribution, but word-level confidence markup,
  word-boundary diarization, and word-level crosstalk removal are faster-whisper
  capabilities. The Lemonade resume-cache policy version is bumped so stale
  token-bearing results are recomputed. Lemonade's bundled ROCm `whisper.cpp` is
  supported directly; no standalone `whisper.cpp` installation is required.
...
- **Lemonade segment timestamps are validated against the audio sent.** Segment
  timestamps stay in one coordinate system and cannot extend beyond a chunk's
  audio. Lemonade `words` entries are intentionally ignored because they are BPE
  tokens rather than reliable lexical word timings.
...
- **The Lemonade "total end-to-end deadline" now actually bounds every blocking network operation, including DNS.** The full timeout went to urllib once and the deadline was only checked between body reads, while synchronous `getaddrinfo()` was outside socket timeouts entirely. DNS now runs in a bounded daemon worker pool, and DNS, each address attempt, proxy CONNECT, TLS, upload, headers, and every body read share the same remaining monotonic budget.
...
- **Invalid Lemonade URLs are never echoed or persisted.** `tapeback status`'s configuration-error branch printed the raw configured `TAPEBACK_LEMONADE_URL`, and the run-log allow-list copied that same raw value before backend validation. Configuration errors now use fixed text, while run logs retain only a normalized `scheme://host[:port]` origin or `[invalid/redacted]`; userinfo, paths, queries, fragments, malformed values, and control characters never cross the persistence boundary.
...
- **Hostile Lemonade segment timestamps can no longer crash or poison the transcript.** Segment timestamps are strictly validated (booleans, NaN/infinity, negatives, and `end < start` are rejected). Every segment-schema failure is a sanitized `LemonadeCapabilityError`, so a malformed response falls back to faster-whisper instead of crashing the run, corrupting sorting/duration math, or persisting invalid timestamps in the resume cache. Chunked responses must also stay within the audio that was actually sent (small boundary slack allowed). Malformed Lemonade BPE-token metadata is ignored.
...
- **HTTPS proxy tunneling can no longer expose the origin request.** The deadline-aware HTTPS connection replaced the standard `connect()` flow without performing urllib's configured CONNECT tunnel, so a proxy could either break remote Lemonade entirely or receive the origin bearer header and multipart audio. Explicit `http://` proxies now receive CONNECT first and origin TLS uses the Lemonade hostname for SNI; proxy credentials stay on CONNECT. TLS-to-proxy (`https://`), scheme-less, and unsupported proxy URLs fail closed before a socket carries credentials or audio.
...
- **A live backend switch now reprocesses committed audio even when the earlier backend emitted no segments.** The transition check used the accumulated segment list as its processed-audio marker, so an empty Lemonade result followed by faster-whisper fallback left the earlier interval permanently tied to the old decoder. Committed byte offsets now trigger the full-session replay; a first-interval fallback remains transactional and avoids redundant work.
...
- **Corrupt-but-valid resume entries are cache misses, not crashes.** `_resume.load()` only guarded key presence, so a syntactically valid but off-schema entry (e.g. `{"segments": [], "info": []}`) loaded fine and then raised `AttributeError` in `transcribe_stereo` — wedging every rerun for that recording. The full persisted schema (object shapes, required fields, finite ordered timestamps, word fields, `info` type) is now validated on load and any failure is treated exactly like a corrupt entry: the channel is recomputed.
- **A runtime CUDA→CPU fallback now changes the faster-whisper identity.** `cache_fingerprint()` encoded only the *requested* device/compute settings, so a fallback that rewrote the resolved device/compute type left the identity untouched: a later CUDA run could reuse output actually produced by CPU/int8, and a live session's backend-switch repair never ran. The resolved device and compute type are now part of the fingerprint (and the isolated worker reports its resolved identity before its result is accepted or cached). One-time consequence: pre-existing resume entries are recomputed on the next run.
- **Live mode no longer commits past a partial result.** `_process_chunk` committed both byte cursors unconditionally, so a backend returning a decoded prefix with `partial=true` left the undecoded suffix of the interval permanently lost from the live note. A partial interval now commits nothing — segments, cursors, and the backend fingerprint stay as they were, and the next cycle retries the whole interval.
- **The exact release head passes `ty check`** (`recorder.py` session-state annotation), and every module is back under the repository's 500-line ceiling: `_lemonade.py` was decomposed into `_lemonade_errors` / `_lemonade_transport` / `_lemonade_audio` / `_lemonade_language` / `_lemonade_url` / `_lemonade_validate`; `live.py` into `_live_pcm` / `_live_chunks`; `transcriber.py` into `_stereo`; `_fw_backend.py` into `_fw_compute`. No public surface changed.
