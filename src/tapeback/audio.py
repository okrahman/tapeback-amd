...
from tapeback._fs import require_fresh_regular_target
...
def merge_channels(monitor_wav: Path, mic_wav: Path, output_dir: Path) -> Path:
    ...
    require_fresh_regular_target(stereo_path, "write the merged stereo file")
...
def split_channels_16k(stereo_wav: Path, output_dir: Path) -> tuple[Path, Path]:
    ...
    require_fresh_regular_target(mic_16k_path, "write the mic channel")
    require_fresh_regular_target(monitor_16k_path, "write the monitor channel")
...
def convert_to_mono16k(input_file: Path, output_dir: Path) -> Path:
    ...
    require_fresh_regular_target(output_path, "write the converted mono file")
